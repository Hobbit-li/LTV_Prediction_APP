...
def data_preprocess(df, train_data=True, num_features=num_features, cat_features=cat_features, 
                    target_col=target_col, id_col=id_col, days_list=days_list,
                    num_features_map=num_features_map, payer_tag=payer_tag):
    """
    ...
    if train_data:
        X_train, X_valid, y_train, y_valid, id_train, id_valid = train_test_split(X, y, user_ids, test_size=0.3, random_state=42)
    else:
        X_train = X_valid = X
        y_train = y_valid = y
    ...
    for i, day in enumerate(days_list):
        ...
        X_train_day_1, X_train_day_2, y_train_day_1, y_train_day_2 = paid_split(X_train_day, y_train_day, payer_tag)
        X_valid_day_1, X_valid_day_2, y_valid_day_1, y_valid_day_2 = paid_split(X_valid_day, y_valid_day, payer_tag)
