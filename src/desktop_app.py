...
class Api:
    def run_model(self, train_file, test_file, ref_month, cost):
        ...
        """
        try:
            logger.info(f"接收到的参数 - train_file: {train_file}, test_file: {test_file}, ref_month: {ref_month}, cost: {cost}")
            ...
            # 运行模型
            logger.info("开始运行模型...")
            results = run_pipeline(train_path_or_error, test_path_or_error, ref_month, cost_value)
            logger.info("模型运行完成")
